...
type
  ...
  private
    ...
    function TBytesToString(B: TBytes): string; overload;
    ...
    class procedure SetCodePage(const Value: Integer); static;
  public
    ...
    class property CodePage: Integer read FCodePage write SetCodePage;
...
begin
  ...
  begin
    E := nil;
    if FCodePage > -1 then
      TEncoding.GetBufferEncoding(B, E, TEncoding.GetEncoding(FCodePage))
    else
      TEncoding.GetBufferEncoding(B, E, TMBCSEncoding.Create);
  end;
  try
...
begin
  if FUTF8Support then
    E := TEncoding.GetEncoding(65001)
  else if FCodePage > -1 then
    E := TEncoding.GetEncoding(FCodePage)
  else
    E := TMBCSEncoding.Create;
...
class procedure TZipFile.SetCodePage(const Value: Integer);
begin
  if TCodePage.Validate(Value) then
    FCodePage := Value;
end;
